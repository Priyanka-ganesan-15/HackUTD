...
result = json.loads(fetch_data(selected_crypto))
print('result:',result.get("message").get("body").get("weights"))
print("dT------------------------------------------------")
...
with col1:
    sentiment_weight = st.slider("Sentiment Weight", 0.0, 2.0, normalized_weights["Sentiment Score"])
    smartcontract_weight = st.slider("Smart Contract Weight", 0.0, 2.0, normalized_weights["Smart Contract Score"])
    regulatory_weight = st.slider("Regulatory Weight", 0.0, 2.0, normalized_weights["Regulatory Score"])

with col2:
    active_address_weight = st.slider("Active Address Weight", 0.0, 1.0, normalized_weights["Active Address Score"])
    whitepaper_weight = st.slider("Whitepaper Weight", 0.0, 2.0, normalized_weights["Whitepaper Exists"])
